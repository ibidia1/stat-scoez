...
import { 
  ...
  Eye, Trophy, Hash, Sparkles, ArrowRight
} from "lucide-react";
          ...
          <div className="space-y-5 p-6 max-h-[70vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl p-3 text-center" style={{
                ...
                <div style={{ fontSize: "11px", color: theme === "dark" ? "#64748b" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "4px" }}>QCM faits</div>
              ...
              <div className="rounded-xl p-3 text-center" style={{
                ...
                <div style={{ fontSize: "11px", color: theme === "dark" ? "#64748b" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "4px" }}>À refaire</div>
                <div style={{ fontSize: "22px", fontWeight: 700, color: getPercentageColor(100 - selectedCourse.toRedo, theme), lineHeight: 1 }}>{selectedCourse.toRedo}%</div>
              </div>
              <div className="rounded-xl p-3 text-center" style={{
                background: theme === "dark" ? "#0f172a" : "#f8fafc",
                border: `1px solid ${theme === "dark" ? "#1e293b" : "#e2e8f0"}`
              }}>
                <div style={{ fontSize: "11px", color: theme === "dark" ? "#64748b" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "4px" }}>Moy. note</div>
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden" style={{
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={history} margin={{ top: 12, right: 16, left: -16, bottom: 4 }}>
                    <defs>
                    ...
                    <CartesianGrid key="dlg-grid" strokeDasharray="3 3" stroke={theme === "dark" ? "#1e293b" : "#f1f5f9"} vertical={false} />
                    <XAxis key="dlg-xaxis" dataKey="date" stroke={theme === "dark" ? "#475569" : "#94a3b8"} style={{ fontSize: "11px" }} tickLine={false} axisLine={{ stroke: theme === "dark" ? "#334155" : "#e2e8f0" }} dy={6} />
                    <YAxis key="dlg-yaxis" domain={[0, 20]} stroke={theme === "dark" ? "#475569" : "#94a3b8"} style={{ fontSize: "11px" }} tickLine={false} axisLine={{ stroke: theme === "dark" ? "#334155" : "#e2e8f0" }} dx={-6} ticks={[0, 5, 10, 15, 20]} />
                    <Tooltip key="dlg-tooltip" content={({ active, payload }: any) => {
                    ...
                    <ReferenceLine key="dlg-refline" y={avg} stroke="#059669" strokeDasharray="6 4" strokeWidth={1.5} />
                    <Area key="dlg-area" type="monotone" dataKey="note" stroke="#4f7cff" strokeWidth={2} fillOpacity={1} fill="url(#dlgGrad)" dot={false} activeDot={{ r: 4, fill: "#4f7cff", strokeWidth: 2, stroke: "#fff" }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            {radarDataForCourse.length > 0 && (
              <div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <RadarChart data={radarDataForCourse}>
                    <PolarGrid stroke={theme === "dark" ? "#334155" : "#e2e8f0"} />
                    <PolarAngleAxis dataKey="tag" tick={{ fill: theme === "dark" ? "#94a3b8" : "#64748b", fontSize: 11 }} />
        ...
        <Card className="relative border-0 overflow-hidden" style={{
          ...
          <motion.div 
            ...
            style={{
              background: filterType === "toRedo"
                ? "linear-gradient(90deg, #dc2626, #ff8f00, #f97316)"
                : "linear-gradient(90deg, #059669, #4f7cff, #8b5cf6)"
            }}
                  ...
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center relative overflow-hidden" style={{
                    ...
                    <div className="text-white text-center relative z-10">
                      <div style={{ fontSize: "20px", fontWeight: 700, lineHeight: 1 }}>
                        {filterType === "toRedo" 
                          ? Math.round(sortedCourses.slice(0, displayCount).reduce((s, c) => s + c.toRedo, 0) / Math.min(displayCount, sortedCourses.length))
                          : Math.round(sortedCourses.slice(0, displayCount).reduce((s, c) => s + c.mastery, 0) / Math.min(displayCount, sortedCourses.length))
                        }
                      </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 style={{ 
                      ...
                      {filterType === "toRedo" ? "Cours à refaire" : "Cours maîtrisés"}
                    </h2>
                    <motion.div
                    ...
                    >
                      {filterType === "toRedo" 
                        ? <Flame size={20} style={{ color: "#ff8f00" }} /> 
                        : <Trophy size={20} style={{ color: "#059669" }} />
                      }
                    </motion.div>
                  </div>
                  <p style={{ color: theme === "dark" ? "#94a3b8" : "#64748b", fontSize: "13px", margin: "4px 0 0 0" }}>
                    Score QE basé sur {Math.min(displayCount, sortedCourses.length)} cours · Cliquez pour explorer
                  </p>
                </div>
              </div>
                  ...
                  {([5, 10, 15] as const).map((count) => (
                    <motion.button
                      ...
                      style={{
                        ...
                        color: displayCount === count 
                          ? "#fff" 
                          : (theme === "dark" ? "#94a3b8" : "#64748b"),
                        background: displayCount === count 
                          ? "#4f7cff" 
                          : "transparent",
                        borderRight: count !== 15 ? `1px solid ${theme === "dark" ? "#334155" : "#e2e8f0"}` : "none",
                      }}
          ...
          <CardContent className="relative pt-0">
            ...
            <div className="flex items-center px-4 py-2 mb-1" style={{
              ...
              <span style={{ width: "36px", fontSize: "11px", fontWeight: 500, color: theme === "dark" ? "#475569" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" }}>#</span>
              <span className="flex-1" style={{ fontSize: "11px", fontWeight: 500, color: theme === "dark" ? "#475569" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" }}>Cours</span>
              <span className="hidden sm:block" style={{ width: "80px", fontSize: "11px", fontWeight: 500, color: theme === "dark" ? "#475569" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", textAlign: "center" }}>QCM</span>
              <span style={{ width: "140px", fontSize: "11px", fontWeight: 500, color: theme === "dark" ? "#475569" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", textAlign: "right" }}>
            ...
            <div className="space-y-0">
              <AnimatePresence mode="popLayout">
                {sortedCourses.slice(0, displayCount).map((course, index) => {
                  const isToRedo = filterType === "toRedo";
                  const value = isToRedo ? course.toRedo : course.mastery;
                  const isExpanded = expandedCourseId === course.name;
                  const history = getHistoryForCourse(course.name, course.mastery);
                  const average = history.reduce((s, h) => s + h.note, 0) / history.length;
                  ...
                  return (
                    ...
                    >
                      ...
                      <motion.div
                        ...
                        onClick={() => setExpandedCourseId(isExpanded ? null : course.name)}
                      >
                        {/* Rank */}
                              ...
                              <div className="px-5 pb-4">
                                ...
                                <div className="rounded-xl overflow-hidden" style={{
                                  ...
                                  <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={history} margin={{ top: 12, right: 16, left: -16, bottom: 4 }}>
                                      <defs key={`inline-defs-${index}`}>
                                        <linearGradient id={`grad-inline-${index}`} x1="0" y1="0" x2="0" y2="1">
                                          <stop offset="5%" stopColor="#4f7cff" stopOpacity={0.35}/>
                                          <stop offset="95%" stopColor="#4f7cff" stopOpacity={0.02}/>
                                        </linearGradient>
                                      </defs>
                                      <CartesianGrid key={`inline-grid-${index}`} strokeDasharray="3 3" stroke={theme === "dark" ? "#1e293b" : "#f1f5f9"} vertical={false} />
                                      <XAxis 
                                        key={`inline-xaxis-${index}`}
                                        dataKey="date" 
                                      ...
                                      />
                                      <Area 
                                        key={`inline-area-${index}`}
                                        type="monotone" 
                                        dataKey="note" 
                                        stroke="#4f7cff"
                                        strokeWidth={2}
                                        fillOpacity={1} 
                                        fill={`url(#grad-inline-${index})`}
                                        dot={false}
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1.5" style={{ fontSize: "12px" }}>
                  ...
                  <span style={{ color: theme === "dark" ? "#94a3b8" : "#64748b" }}>Critique</span>
                </span>
                <span className="flex items-center gap-1.5" style={{ fontSize: "12px" }}>
                  <div className="w-2.5 h-2.5 rounded-full" style={{ background: "#ff8f00" }} />
                  <span style={{ color: theme === "dark" ? "#94a3b8" : "#64748b" }}>Moyen</span>
                </span>
                <span className="flex items-center gap-1.5" style={{ fontSize: "12px" }}>
                  <div className="w-2.5 h-2.5 rounded-full" style={{ background: "#059669" }} />
                  <span style={{ color: theme === "dark" ? "#94a3b8" : "#64748b" }}>Bon</span>
                </span>
              </div>
            </motion.div>
          </CardContent>
